...
Set the model endpoint and embedding settings in `config/qa_agent.toml`, then run `./run.ps1`. The browser service listens on `8080`; the Qwen model endpoint must be a separate port such as `11440`. Open `http://127.0.0.1:8080` for the browser test console. Each message shows its timestamp and the answer's elapsed time.
