...
def _kmeans_labels(normed: np.ndarray, k: int) -> np.ndarray:
    """Run the pinned K-means algorithm; errors must remain visible."""
...
def cluster_level_soft(
    ...
    if not primary:
        ...
        # Upstream re-appends every orphan here, duplicating all memberships.
        # All indices already belong to the sole fallback cluster.
        orphans = []
...
def build_hierarchy(
    ...
    """
    n = len(doc_ids)
    if n == 0 or n != embeddings.shape[0] or n != len(doc_texts):
        raise ValueError("Nonempty, aligned corpus inputs are required")
    if initial_summaries is None or len(initial_summaries) != n:
        raise ValueError("Full initial summaries are required in the comparison")
    if doc_cards:
        raise ValueError("Document cards are replaced by shared comparison descriptions")
    if not (summarize_members_fn or summarize_fn or summarize_batch_fn) or not callable(embed_fn):
        raise ValueError("Explicit summary and embedding callbacks are required")

    leaf_nodes = []
    for i, (did, txt) in enumerate(zip(doc_ids, doc_texts)):
        card = (doc_cards or {}).get(did)
        leaf_summary = initial_summaries[i]
        if card:
    ...
    while len(current_nodes) > max_top:
        ...
        if not prim_assign or len(prim_assign) >= len(current_nodes):
            raise ValueError("Clustering did not reduce the current level")
        if sorted(i for members in prim_assign for i in members) != list(range(len(current_nodes))):
            raise ValueError("Clustering omitted or duplicated member indices")
        ...
        def _summarize(clusters_to_run: list[dict]) -> None:
            ...
            if summarize_members_fn:
                summaries = [summarize_members_fn(
                    [current_nodes[i] for i in c["member_indices"]], level=level
                ) for c in clusters_to_run]
            elif summarize_batch_fn:
                summaries = summarize_batch_fn(inputs, level=level)
            elif summarize_fn:
                summaries = [summarize_fn(si, level=level) for si in inputs]
            else:
                raise ValueError("Missing summary callback")
            if len(summaries) != len(clusters_to_run) or any(not s for s in summaries):
                raise ValueError("Summary callback omitted a cluster")
        ...
        if repartition_fn is not None:
            ...
            if records:
                try:
                    ...
                    # Comparison failures, including capacity errors, must be visible.
                    raise

        if not clusters or len(clusters) >= len(current_nodes):
            raise ValueError("Repartition did not reduce the current level")
        if sorted(i for c in clusters for i in c['member_indices']) != list(range(len(current_nodes))):
            raise ValueError("Repartition omitted or duplicated member indices")
        ...
        for ci, c in enumerate(clusters):
            ...
            summary = c["summary"]
            if not summary:
                raise ValueError("Missing cluster summary")
            if embed_fn:
                ...
                raise ValueError("Missing embedding callback")
